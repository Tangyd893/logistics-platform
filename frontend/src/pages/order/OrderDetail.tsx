  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (orderRes.data.code === 200) {
        const full = { ...orderRes.data.data, logs: logsRes.data.code === 200 ? logsRes.data.data : [] }
        setOrder(full)
      }
  ...
  return (
    <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
          ...
          <tbody>
            {order.items.map((item) => (
              <tr key={item.id}>
                <td style={colStyle}>{item.skuName}</td>
                <td style={colStyle}>{item.skuCode}</td>
      ...
      <div style={{ ...infoCardStyle, marginTop: '1rem' }}>
        ...
        {order.logs.length === 0 ? (
          <div style={{ color: '#94a3b8', fontSize: '0.875rem' }}>暂无日志</div>
        ) : (
              ...
              return (
                <div key={log.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0', borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>
                    {log.operateTime?.slice(0, 16).replace('T', ' ')} {log.operatorName || log.operateBy || ''}
                  </div>
                </div>
              )
            })}
